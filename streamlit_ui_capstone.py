...
model = genai.GenerativeModel(
  ...
  system_instruction="Act as an experienced blogger who knows about all the topics in this world. You will be receiving a blog post topic or an image as an input and you have to write a crisp and humanly toned blog post. Do not make up fake stuff and try to be very factual and quote stats related to that topic if possible to make the end result accurate. Keep the word count at 150 words. Do not cross this word limit. If the input is an image then try to detect the topic from the image and write a blog post on it. If it is a celebrity, try recognising them and write about them. If it is an object, then write a blog on them.You are expected to output a heading for the blog post and the actual blog post as response. If you are not getting an image then say I am unable to write a blog on this image.",
)
...
def process_gemini_response(prompt):
    user_message = Message("User : "+prompt,'text')
    st.session_state.chat.append(user_message)
    # Gemini Integration here
    chat_session = model.start_chat()
    response=chat_session.send_message(prompt)
    gemini_message = Message("Gemini : "+response.text,'text')
